{#- Pre-check body: file validation -#}
section "Files"{% if account is defined and account.pullSecret is defined %}
...
fi{% endif %}{% for key in cluster.sshKeys | default([]) | unique %}
if [ -f "{{ key }}" ]; then
    ssh-keygen -l -f "{{ key }}" &>/dev/null && pass "{{ key }} valid SSH key" || warn "{{ key }} invalid format"
else
    warn "{{ key }} not found"
fi{% endfor %}{% if network.trustBundle is defined %}
