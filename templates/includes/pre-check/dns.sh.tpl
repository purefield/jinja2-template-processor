{#- Pre-check body: DNS forward/reverse lookups and resolvers -#}
section "DNS Forward"{% if network.primary is defined and network.primary.vips is defined %}
...
done{% if network.primary.vips.apps is defined %}
result=$(dig +short test.apps.{{ cluster.name }}.{{ network.domain }} 2>/dev/null | tail -1)
[ -n "$result" ] && pass "*.apps.{{ cluster.name }}.{{ network.domain }} → wildcard OK" || warn "*.apps wildcard not configured"{% endif %}{% endif %}{% for name, host in (hosts | default({})).items() %}
...
[ -n "$result" ] && pass "{{ host.network.primary.address }} → $result" || warn "{{ host.network.primary.address }} no PTR"{% endfor %}{% endif %}{% for ns in network.nameservers | default([]) %}{% if loop.first %}

section "DNS Resolvers"{% endif %}
dig @"{{ ns }}" +short +time=2 google.com &>/dev/null && pass "{{ ns }} responding" || warn "{{ ns }} not responding"{% endfor %}
