...
for h in {% if network.primary.vips %}{{ network.primary.vips.api|as_list|join(' ') }} {{ network.primary.vips.apps|as_list|join(' ') }}{% endif %}{% for name,host in hosts.items() %} {{ host.network.primary.address }}{% endfor %}; do echo -n "* $h -> "; dig +short -x $h; echo; done
